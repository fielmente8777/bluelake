import { Metadata } from "next";
import { SectionWithContainer } from "@/components/sectionComponants";
import { Header } from "@/components/Header";
import { Footer } from "@/components/Footer";
import { policiesPageData } from "./privacyData";

const privacyPolicyItem =
  policiesPageData.find((p) => p.slug === "privacy-policy") ||
  policiesPageData[0];

export const metadata: Metadata = {
  title: privacyPolicyItem.metaData.title,
  description: privacyPolicyItem.metaData.description,
};

export default function PrivacyPolicyPage() {
  return (
    <>
      {/* ================= HEADER ================= */}
      <Header />

      {/* ================= MAIN PAGE ================= */}
      <main className="font-sans text-navy-deep">

        {/* ================= HERO / HEADER ================= */}
        <SectionWithContainer
          sectionClassName="relative overflow-hidden bg-navy-deep"
          defaultPadding={false}
        >
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.06),transparent_60%)]" />

          <div className="relative flex min-h-[300px] flex-col items-center justify-center px-5 py-16 text-center md:min-h-[340px] md:py-20">

            {/* GOLD LABEL */}
            <div className="mb-5 flex items-center justify-center gap-3">
              <span className="h-px w-[28px] bg-gold" />

              <span className="text-[13px] font-bold tracking-[0.18em] text-gold uppercase">
                Privacy Policy
              </span>

              <span className="h-px w-[28px] bg-gold" />
            </div>

            {/* TITLE */}
            <h1 className="max-w-[900px] font-display text-[36px] leading-[1.1] font-medium text-white md:text-[52px]">
              {privacyPolicyItem.title}
            </h1>

            {/* BRAND */}
            <p className="mt-5 text-[14px] font-semibold tracking-[0.14em] text-white/75 uppercase">
              The Blue Lake, Igatpuri
            </p>

          </div>
        </SectionWithContainer>

        {/* ================= CONTENT ================= */}
        <SectionWithContainer
          sectionClassName="bg-[#f8f9f3] py-[55px] md:py-[75px]"
          containerClassName="max-w-[1050px]"
          defaultPadding={false}
        >
          <article
            className="
              rounded-sm
              bg-white
              px-6
              py-8
              shadow-[0_2px_18px_rgba(11,37,69,0.06)]

              sm:px-9
              sm:py-10

              md:px-12
              md:py-12

              text-[15px]
              leading-[1.8]
              text-[#52657d]

              [&_h3]:mt-10
              [&_h3]:mb-4
              [&_h3]:font-display
              [&_h3]:text-[25px]
              [&_h3]:font-medium
              [&_h3]:leading-[1.3]
              [&_h3]:text-navy-deep
              [&_h3:first-child]:mt-0

              [&_h4]:mt-7
              [&_h4]:mb-3
              [&_h4]:font-sans
              [&_h4]:text-[18px]
              [&_h4]:font-semibold
              [&_h4]:text-navy-deep

              [&_p]:mb-4

              [&_ul]:mb-5
              [&_ul]:list-disc
              [&_ul]:space-y-2
              [&_ul]:pl-6

              [&_ol]:mb-5
              [&_ol]:list-decimal
              [&_ol]:space-y-2
              [&_ol]:pl-6

              [&_li]:pl-1

              [&_a]:font-medium
              [&_a]:text-navy-deep
              [&_a]:underline
              [&_a]:underline-offset-2
              [&_a]:decoration-gold
              [&_a:hover]:text-gold

              [&_strong]:font-semibold
              [&_strong]:text-navy-deep

              md:text-[16px]
            "
            dangerouslySetInnerHTML={{
              __html: privacyPolicyItem.content,
            }}
          />
        </SectionWithContainer>

        {/* ================= BOTTOM DECORATION ================= */}
        <div className="h-[5px] bg-navy-deep" />

      </main>

      {/* ================= FOOTER ================= */}
      <Footer />
    </>
  );
}